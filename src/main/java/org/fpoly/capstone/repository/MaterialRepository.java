...
public interface MaterialRepository extends JpaRepository<Material, Long> {
    ...
    @Query("SELECT new org.fpoly.capstone.service.payload.material.MaterialResponse(m.id, m.name) " +
            "FROM Material m " +
            "WHERE (:name IS NULL OR LOWER(m.name) LIKE LOWER(CONCAT('%', :name, '%'))) ")
    Page<MaterialResponse> findByFilter(@Param("name") String name, Pageable pageable);
