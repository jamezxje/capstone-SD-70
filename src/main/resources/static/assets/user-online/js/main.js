...
(function ($) {
    ...
    function price_slider() {
        $("#slider-range").slider({
            range: true,
            min: 100,   // Đặt min là 100
            max: 1000,  // Đặt max là 1 triệu
            values: [100, 1000],  // Giá trị mặc định
            slide: function (event, ui) {
                // Cập nhật giá trị hiển thị khi thay đổi slider
                $("#amount").val(ui.values[0] + " - " + ui.values[1]);

                // Cập nhật giá trị ẩn cho minPrice và maxPrice
                $("#minPrice").val(ui.values[0]);
                $("#maxPrice").val(ui.values[1]);
            }
        });

        // Hiển thị giá trị slider ban đầu
        $("#amount").val($("#slider-range").slider("values", 0) +
            " - " + $("#slider-range").slider("values", 1));

        // Xử lý nhập thủ công vào ô #amount
        $("#amount").on("change", function () {
            var value = $(this).val();
            var values = value.split(" - ");

            if (values.length === 2) {
                var minVal = parseInt(values[0].replace(/[^0-9]/g, ""), 10);
                var maxVal = parseInt(values[1].replace(/[^0-9]/g, ""), 10);

                // Cập nhật giá trị cho minPrice và maxPrice nếu có giá trị hợp lệ
